    ...
    <section className="min-h-0 flex-1 overflow-y-auto pr-1">
      ...
      <article className={`${cardClass} max-w-xl`}>
        ...
        <form onSubmit={saveProfile} className="grid gap-3">
          ...
          <input className={inputClass} aria-label="Recruiting contact email" placeholder="Recruiting contact email" value={profile.recruiter_email} onChange={(e) => setProfile({ ...profile, recruiter_email: e.target.value })} required />
          <button className={buttonClass} type="submit" aria-busy={loading.profile} disabled={loading.profile}>
